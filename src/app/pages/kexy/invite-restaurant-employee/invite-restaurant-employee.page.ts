...
@Component({
  selector: 'app-invite-restaurant-employee',
  templateUrl: './invite-restaurant-employee.page.html',
  styleUrls: ['./invite-restaurant-employee.page.scss'],
})
export class InviteRestaurantEmployeePage extends BasePage implements OnInit {
  ...
  ngOnInit() {
    this.restaurant_id = this.params.restaurant_id ; // TODO
    ...
    if (this.from_page !== 'my_distributor') {
      this._disableMenu();
    }
  }
  ...
  async primaryFormSubmitted(): Promise<void> {
    ...
    let res = await this.callApi(apis.API_RESTAURANT_INVITE_EMPLOYEES, data);
    if (!res.success) {
      return;
    }
