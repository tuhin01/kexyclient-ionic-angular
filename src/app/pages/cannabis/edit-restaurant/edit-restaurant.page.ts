...
export class EditRestaurantPage extends BasePage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.restaurantEditForm = new FormGroup({
      side: new FormControl("FOH", Validators.compose([Validators.required])),
  ...
  async getCompanyData(): Promise<any> {
    ...
    if (this.orgInfo.logo_image_url)
      this.imageUrl = this.baseUriForImages + this.orgInfo.logo_image_url;
